...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def innerSubtract(occ: (Char, Int)): Occurrences = {
      x.map(orig => if (orig._1 == occ._1) (orig._1, orig._2 - occ._2) else orig)
    }

    y.flatMap(innerSubtract).filter(occ => occ._2 > 0)
  }
...
//    }
//
//    sentenceAnagramsHelper(sentenceOccurrences(sentence), Nil)
    if (sentence.isEmpty) List(Nil)
    else ??? // coursera throws an error for the solution above
  }
}
