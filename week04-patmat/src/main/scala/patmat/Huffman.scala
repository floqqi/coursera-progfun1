...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def calc(chars: List[Char], res: List[(Char, Int)]): List[(Char, Int)] = {
      if (chars.isEmpty) res
      else {
        val i = res.indexWhere(p => p._1 == chars.head)
        calc(
          chars.tail,
          if (i == -1) res :+ (chars.head, 1)
          else res.updated(i, (chars.head, res(i)._2 + 1))
        )
      }
    }
    calc(chars, Nil)
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    def makeList(freqs: List[(Char, Int)], res: List[Leaf]): List[Leaf] = {
      if (freqs.isEmpty) res
      else makeList(freqs.tail, res :+ Leaf(freqs.head._1, freqs.head._2))
    }
    makeList(freqs, Nil).sortWith((a, b) => a.weight < b.weight)
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    traverse(tree, bits)
    //    def f(tree: CodeTree, bits: List[Bit], res: List[Char]): List[Char] = tree match {
    //      case Leaf(char, _) => if (bits == Nil) res :+ char else f(tree, bits, res :+ char)
    //      case Fork(left, right, _, _) => f(if (bits.head == 0) left else right, bits.tail, res)
    //    }
    //    f(tree, bits, Nil)
  }
